function make_slides(f) {
  ...
  function buildScenarioText(stim, altShuffledOrder) {
    ...
    // pragmatics7: reference-game items carry a `display` (array of
    // {color, shape} objects) instead of relying on scenario text alone.
    // Render it inline, right after the prose, and before the
    // alternatives/utterance block below. Order is shuffled per-render so
    // left/right position never hints at which object is the target — the
    // target is never highlighted here (that would give away the answer).
    if (stim.display && stim.display.length) {
      scenarioText += buildRefgameDisplayHTML(_.shuffle(stim.display.slice()));
    }
  ...
  function log_responses(stim, rationale, slider_value, condition, leftInterpIdx, rightInterpIdx, altShuffledOrder) {
    ...
    let trial_data = {
      ...
      stimulus_type: stim.stimulus_type || 'natural', // 'natural' (p4 dialogue) or 'refgame' (object display)
      condition: condition,
      ...
      // refgame-only metadata (null for natural-language items)
      context_id: stim.context_id || null,
      display: stim.display ? stim.display.map(o => o.color + '_' + o.shape).join(', ') : null,
      target: stim.target ? (stim.target.color + '_' + stim.target.shape) : null,
      competitor: stim.competitor ? (stim.competitor.color + '_' + stim.competitor.shape) : null,
      weak_utterance: stim.weak_utterance || null,
      strong_utterance: stim.strong_utterance || null,
      rationale: rationale,
...
function init() {
  ...
  // Second, independent coin flip for the object-display warmup pair
  var warmup_coin_refgame = Math.random() < 0.5;
  exp.warmup_stimuli.push(
    warmup_coin_refgame
