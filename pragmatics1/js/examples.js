
// all the examples data which comes from src/human_exp/pragmatics1/DeceitsExamples.csv 

var examples_deceits = [
    {
        "id": "examples_1",
        "scenario": "At a family gathering, Liam tries a cookie from the dessert table. He grimaces and puts the unfinished cookie back down on the table while no one is looking. As he turns around, his aunt, who baked the cookies, asks him how he finds them. Liam responds: \"They're great!\"",
        "question": "Why has Liam responded in such a way?",
        "interpretations": [
            "He believes the cookies are delicious.",
            "He wants his aunt to think he didn't eat the cookies.",
            "He is trying to be polite by not criticizing his aunt's cooking.",
            "He thinks the cookies do not taste good."
        ]
    }
];
// all the examples data which comes from src/human_exp/pragmatics1/IndirectSpeechExamples.csv 

var examples_indirectspeech = [
    {
        "id": "examples_1",
        "scenario": "During dinner with his family, Marcus' mother asks him what he thinks of the food. He glances toward the salt shaker on the other end of the table and says aloud, \"It would be great if it had more salt.\"",
        "question": "What might he be trying to convey?",
        "interpretations": [
            "He finds the food bland.",
            "He wants his mom to pass him the salt.",
            "He prefers when his food is on the saltier side.",
            "He's reminding his mother how she doesn't put enough salt in her food."
        ]
    }
];
// all the examples data which comes from src/human_exp/pragmatics1/IronyExamples.csv 

var examples_irony = [
    {
        "id": "examples_1",
        "scenario": "Bruce and Vanessa, two friends from work, are taking a walk in the park. The weather is beautiful, and they are enjoying the fresh air. Bruce turns to Vanessa and says, \"Isn't it so ugly out?\"",
        "question": "What did Bruce want to convey?",
        "interpretations": [
            "He thinks the weather outside is terrible.",
            "He thinks the weather outside is amazing.",
            "He thinks it's going to rain soon.",
            "He wants to take Vanessa out to dinner."
        ]
    }
];
// all the examples data which comes from src/human_exp/pragmatics1/MaximsExamples.csv 

var examples_maxims = [
    {
        "id": "examples_1",
        "scenario": "John and his girlfriend Sarah are watching a movie together. It's Sarah's favorite movie and she has been dying to see it again with him. After it ends, Sarah asks John if he liked it. John says, \"I think the narrator had a really nice voice.\"",
        "question": "Why had John responded like this?",
        "interpretations": [
            "John does not want to be truthful and upset Sarah.",
            "John wants to go to bed.",
            "John liked the voice narration in the movie.",
            "John did not like the movie."
        ]
    }
];
// all the examples data which comes from src/human_exp/pragmatics1/MetaphorExamples.csv 

var examples_metaphor = [
    {
        "id": "examples_1",
        "scenario": "Leah and Martin agreed to meet at seven, but it is now seven twenty. Leah, looking annoyed, says to herself, \"That man is like a broken clock.\"",
        "question": "What does Leah mean?",
        "interpretations": [
            "Martin is made of gears and metal.",
            "Martin is always late to meetings.",
            "Martin is emotionally fragile and unreliable.",
            "She is remembering the time they visited an antique shop.",
            "She's annoyed by Martin's tardiness."
        ]
    }
];
